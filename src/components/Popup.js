...
const Popup = (props) => {
  ...
  return (
    <div
      id="popup"
      className={`popup ${hidden} z-40 bg-white text-xl text-slate-800`}
    >
      <div className="popHeading sticky top-2">
        <h1>{project_name}</h1>
        <span className="mt-1">
      ...
      <div className="poptoolTechs p-1 md:p-3">
        <p className="float-left">Tools and Technologies :</p>
        {technologies &&
          technologies.map((techs) => <li key={techs}>{techs}</li>)}
        {other_techs &&
          other_techs.map((techs) => <li key={techs}>{techs}</li>)}
      </div>
      ...
      <div id="popImages">
        {project_imgs &&
          project_imgs.map((img) => (
            <div key={img} className="popImagesimg">
